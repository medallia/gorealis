...
func (p *AuroraAdminClient) recvPruneTasks() (value *Response, err error) {
	...
	if mTypeId == thrift.EXCEPTION {
		error364 := thrift.NewTApplicationException(thrift.UNKNOWN_APPLICATION_EXCEPTION, "Unknown Exception")
		var error365 error
...
func NewAuroraAdminProcessor(handler AuroraAdmin) *AuroraAdminProcessor {
	self366 := &AuroraAdminProcessor{NewAuroraSchedulerManagerProcessor(handler)}
	self366.AddToProcessorMap("setQuota", &auroraAdminProcessorSetQuota{handler: handler})
	self366.AddToProcessorMap("forceTaskState", &auroraAdminProcessorForceTaskState{handler: handler})
	self366.AddToProcessorMap("performBackup", &auroraAdminProcessorPerformBackup{handler: handler})
	self366.AddToProcessorMap("listBackups", &auroraAdminProcessorListBackups{handler: handler})
	self366.AddToProcessorMap("stageRecovery", &auroraAdminProcessorStageRecovery{handler: handler})
	self366.AddToProcessorMap("queryRecovery", &auroraAdminProcessorQueryRecovery{handler: handler})
	self366.AddToProcessorMap("deleteRecoveryTasks", &auroraAdminProcessorDeleteRecoveryTasks{handler: handler})
	self366.AddToProcessorMap("commitRecovery", &auroraAdminProcessorCommitRecovery{handler: handler})
	self366.AddToProcessorMap("unloadRecovery", &auroraAdminProcessorUnloadRecovery{handler: handler})
	self366.AddToProcessorMap("startMaintenance", &auroraAdminProcessorStartMaintenance{handler: handler})
	self366.AddToProcessorMap("drainHosts", &auroraAdminProcessorDrainHosts{handler: handler})
	self366.AddToProcessorMap("maintenanceStatus", &auroraAdminProcessorMaintenanceStatus{handler: handler})
	self366.AddToProcessorMap("endMaintenance", &auroraAdminProcessorEndMaintenance{handler: handler})
	self366.AddToProcessorMap("slaDrainHosts", &auroraAdminProcessorSlaDrainHosts{handler: handler})
	self366.AddToProcessorMap("snapshot", &auroraAdminProcessorSnapshot{handler: handler})
	self366.AddToProcessorMap("triggerExplicitTaskReconciliation", &auroraAdminProcessorTriggerExplicitTaskReconciliation{handler: handler})
	self366.AddToProcessorMap("triggerImplicitTaskReconciliation", &auroraAdminProcessorTriggerImplicitTaskReconciliation{handler: handler})
	self366.AddToProcessorMap("pruneTasks", &auroraAdminProcessorPruneTasks{handler: handler})
	return self366
}
